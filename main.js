...
function handleAnswer(e) {
  ...
  document.querySelectorAll('.answer-btn').forEach(btn => {
    btn.disabled = true;
    btn.style.backgroundColor = btn.dataset.correct === "true" ? "#4caf50" : "#f44336";
  });

  const nextButton = document.createElement('button');
  nextButton.textContent = currentIndex < questions.length - 1 ? "Próxima" : "Ver Resultado";
...
function getMessage(score) {
  if (score === questions.length) return "Você é o Rei dos Piratas!";
  if (score >= questions.length * 0.7) return "Ótimo desempenho, Mugiwara!";
  if (score >= questions.length * 0.4) return "Você está no caminho certo.";
  return "Talvez precise rever os episódios!";
}
...
function goBack() {
  window.location.href = "quizonepiece.html";
}
